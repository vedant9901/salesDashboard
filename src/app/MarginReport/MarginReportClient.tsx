// src/app/MarginReport/MarginReportClient.tsx
"use client";
...
export default function MarginReportClient({
  ...
  const departmentList = momData && momData.length > 0
    ? Array.from(
        new Set(
          momData.flatMap((m: any) =>
            (m.Departments || []).map((d: any) => d.Department),
          ),
        ),
      )
    : [];
  ...
  useEffect(() => {
    if (momData?.length > 0 && !selectedDepartment && !hasUserCleared) {
      const firstDept =
        ...
        momData.flatMap((m: any) => m.Departments.map((d: any) => d.Department))[0] ||
        null;
  ...
  }, [initialDepartment, selectedMoMDept]);
  ...
  // KPI calculations (safe with optional chaining and defaults)
  ...
  const dataToRender = useMemo(() => {
    ...
    if (!selectedDepartment) {
      // return department-level items
      return items;
    }

    const deptData = items.find((d: any) => d.Department === selectedDepartment);
  ...
  // Department/SubClass table renderer
        ...
        <table className="w-full table-auto border-collapse text-sm">
          <thead className="sticky top-0 bg-gray-100 text-xs font-semibold uppercase text-gray-700">
            <tr>
              ...
              <th className="border border-gray-300 px-4 py-2 text-center" colSpan={5}>
                {startDate} - {endDate}
              </th>
              <th className="border border-gray-300 px-4 py-2 text-center" colSpan={4}>
                Last Month
              </th>
              <th className="border border-gray-300 px-4 py-2 text-center" colSpan={4}>
                Last Year
            ...
            <tr>
              {["Qty", "Amount (₹)", "COGS (₹)", "GM (₹)", "Margin (%)"].map((t, i) => (
                <th key={`c${i}`} className="border border-gray-300 bg-blue-50 px-4 py-2 text-right">
                  {t}
                </th>
              ))}
              {["Qty", "Amount (₹)", "GM (₹)", "Margin (%)"].map((t, i) => (
                <th key={`lm${i}`} className="border border-gray-300 bg-yellow-50 px-4 py-2 text-right">
                  {t}
                </th>
              ))}
              {["Qty", "Amount (₹)", "GM (₹)", "Margin (%)"].map((t, i) => (
                <th key={`ly${i}`} className="border border-gray-300 bg-green-50 px-4 py-2 text-right">
                  {t}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {finalData.map((row: any, idx: number) => {
              const currentRGM = calculateRGM(row.GrossMarginAmount ?? 0, row.TotalAmount ?? 0);
              const lastMonthRGM = calculateRGM(row.LastMonthGrossMargin ?? 0, row.LastMonthTotalAmount ?? 0);
              const lastYearRGM = calculateRGM(row.LastYearGrossMargin ?? 0, row.LastYearTotalAmount ?? 0);

              const YoY = row.LastYearTotalAmount
                ? ((row.TotalAmount - row.LastYearTotalAmount) / row.LastYearTotalAmount) * 100
                : 0;

              const MoM = row.LastMonthTotalAmount
                ? ((row.TotalAmount - row.LastMonthTotalAmount) / row.LastMonthTotalAmount) * 100
                : 0;

              const isGrandTotal = row.Department === "Grand Total";
              const rowBg = isGrandTotal ? "bg-orange-100 font-bold" : idx % 2 === 0 ? "bg-white" : "bg-gray-50";

              return (
                <tr key={idx} className={`${rowBg} border-t border-gray-200`}>
                  <td className={cn("sticky left-0 min-w-[200px] border border-gray-300 px-4 py-2 font-medium", rowBg)}>
                  ...
                  <td className={cn("border border-gray-300 px-4 py-2 text-right font-semibold", getRGMColor(currentRGM))}>
                  ...
                  <td className={cn("border border-gray-300 px-4 py-2 text-right font-semibold", getRGMColor(lastMonthRGM))}>
                  ...
                  <td className={cn("border border-gray-300 px-4 py-2 text-right font-semibold", getRGMColor(lastYearRGM))}>
                    {standardFormat(lastYearRGM)}%
                  </td>

                  <td className={cn("border border-gray-300 px-4 py-2 text-right font-semibold", getRGMColor(YoY))}>
                    {standardFormat(YoY)}%
                  </td>
                  <td className={cn("border border-gray-300 px-4 py-2 text-right font-semibold", getRGMColor(MoM))}>
                    {standardFormat(MoM)}%
                  </td>
                </tr>
              );
  ...
  if (status === "loading") {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <div className="loader h-16 w-16 rounded-full border-8 border-t-8 border-gray-200 ease-linear" />
      </div>
    );
  }
      ...
      {toggleValue === 0 ? (
        <div className="space-y-6 p-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {kpiCards.map((kpi) => {
              const MoM = kpi.lastMonth ? ((kpi.value - kpi.lastMonth) / kpi.lastMonth) * 100 : 0;
              const YoY = kpi.lastYear ? ((kpi.value - kpi.lastYear) / kpi.lastYear) * 100 : 0;
              return (
                <div
                  key={kpi.title}
                ...
                >
                  ...
                  <div className="text-xl font-bold text-gray-900">
                    {kpi.title === "Margin %" ? `${standardFormat(kpi.value)}%` : `₹ ${standardFormat(kpi.value)}`}
                  </div>
                  <div className="flex justify-between text-xs text-gray-500">
                    <span>MoM</span>
                    <span className={getRGMColor(MoM)}>{standardFormat(MoM)}%</span>
                  </div>
                  <div className="flex justify-between text-xs text-gray-500">
                    <span>YoY</span>
                    <span className={getRGMColor(YoY)}>{standardFormat(YoY)}%</span>
                  </div>
                </div>
              );
            })}
          ...
          <MixBarChart
            data={sortedData.map((d: any) => ({
              name: d.Department || d.SubClass || "Unknown",
              Current: d.TotalAmount ?? 0,
              LastMonth: d.LastMonthTotalAmount ?? 0,
              LastYear: d.LastYearTotalAmount ?? 0,
              Margin: d.TotalAmount ? ((d.GrossMarginAmount ?? 0) / d.TotalAmount) * 100 : 0,
            }))}
            height={300}
          />
        ...
        <div className="space-y-6 p-4">
          ...
          <div className="relative flex max-h-[85vh] flex-col rounded-2xl border border-gray-200 bg-white shadow-xl">
            <div className="sticky top-0 flex items-center justify-between border-b bg-gradient-to-r from-blue-50 to-blue-100 px-4 py-3 shadow-sm">
              <h2 className="text-lg font-semibold text-gray-800">Department: {selectedMoMDept || "—"}</h2>

              <div className="flex items-center gap-3">
                {currentPage > 1 && (
                  <button
                    onClick={() => onPreviousPage && onPreviousPage(selectedMoMDept)}
            ...
            <div className="flex-1 overflow-auto">
              {selectedMoMDept && momData.length > 0 ? (
                (() => {
                  const months = momData.map((m: any) => formatMonthName(m.Month));

                  // typed brandMap so indexing is allowed
                  const brandMap: Record<string, Record<string, { sale: number; margin: number }>> = {};

                  momData.forEach((month: any) => {
                    const monthName = formatMonthName(month.Month);

                    const dept = (month.Departments || []).find((d: any) => d.Department === selectedMoMDept);

                    (dept?.Brands || []).forEach((brand: any) => {
                      const name = (brand?.Brand && String(brand.Brand).trim()) || "(No Name)";

                      const sale = brand?.GrossAmount ?? 0;
                      const cogs = brand?.COGS ?? 0;
                      const margin = sale > 0 ? ((sale - cogs) / sale) * 100 : 0;
                      ...
                      <table className="w-full border-collapse text-sm text-gray-800">
                        <thead className="sticky top-0 z-40 bg-gray-100 shadow-sm">
                          <tr>
                            ...
                            <th colSpan={months.length} className="whitespace-nowrap border-b border-gray-200 bg-gray-100 px-2 py-2 text-center font-semibold">
                              Sale (₹)
                            </th>
                          ...
                          <tr className="bg-gray-50 text-gray-600">
                            ...
                            {months.map((monthName, i) => (
                              <th key={`sale-header-${monthName}-${i}`} className="whitespace-nowrap border-b px-2 py-2 text-center font-medium">
                                {monthName}
                              </th>
                            ))}

                            {months.map((monthName, i) => (
                              <th key={`margin-header-${monthName}-${i}`} className="whitespace-nowrap border-b px-2 py-2 text-center font-medium">
                                {monthName}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {Object.entries(brandMap as Record<string, Record<string, { sale: number; margin: number }>>).map(
                            ([brandName, monthData], idx) => (
                              <tr key={brandName} className={`border-b transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-blue-50`}>
                                <td className="sticky left-0 z-10 whitespace-nowrap border-r bg-inherit px-2 py-2 font-medium text-gray-900">
                                  {brandName}
                                </td>

                                {months.map((monthName, i) => (
                                  <td key={`sale-${brandName}-${monthName}-${i}`} className="whitespace-nowrap px-2 py-2 text-right font-semibold text-gray-900">
                                    ₹ {standardFormat(monthData[monthName]?.sale ?? 0)}
                                  </td>
                                ))}

                                {months.map((monthName, i) => (
                                  <td key={`margin-${brandName}-${monthName}-${i}`} className={cn("whitespace-nowrap px-2 py-2 text-center font-semibold", getRGMColor(monthData[monthName]?.margin ?? 0))}>
                                    {standardFormat(monthData[monthName]?.margin ?? 0)}%
                                  </td>
                                ))}
                              </tr>
                            ),
                ...
                <div className="flex h-full items-center justify-center p-6 text-gray-500">No data found for this department.</div>
              )}
            </div>
          </div>
        </div>
